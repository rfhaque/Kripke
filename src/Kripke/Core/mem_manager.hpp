...
/// Memory types supported by Kripke.
enum class MemoryType
{
   HOST,           /**< Host memory; using an Umpire allocator which can be set
                        with MemoryManager::SetUmpireHostAllocatorName */
   MANAGED,        /**< Managed memory; using CUDA or HIP *MallocManaged
                        and *Free */
   DEVICE,         /**< Device memory; using an Umpire allocator which can be
                        set with MemoryManager::SetUmpireDeviceAllocatorName */
   SIZE,
};

extern const char *MemoryTypeName[MemoryType::SIZE];
